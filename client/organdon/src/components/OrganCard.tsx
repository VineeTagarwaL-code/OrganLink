...
const OrganCard = ({
    ...
    async function getInstituteDetails() {
        const res = await apiConnector("GET", userEndpoints.GET_USER+"/"+instituteId, null, {
    ...
    function handleChat(){
        navigate({pathname: `/chat/${_id}`,search: "?name="+instituteDetail.name})
    }
    ...
    return (
        <div>
            <Card className="overflow-y-scroll bg-[#1F2937] rounded-xl w-96 p-5 items-center flex flex-col hover:bg-white hover:text-black font-serif">
                ...
                <div className="flex gap-x-4 items-center">
                    ...
                    <div className="flex flex-col ml-4">
                        ...
                        <p className="text-lg">
                            {bloodGroup[0].toUpperCase() +
                                bloodGroup.slice(1).toLowerCase()}
                        </p>
                        <p className="text-lg">
                        ...
                        <p className="text-[14px] text-zinc-400">
                            {calculateDaysDifference(createdAt) > 0 ? (
                                <p>
                                    {calculateDaysDifference(createdAt)}{' '}
                                    days ago
                                </p>
                            ) : (
                                <p>Today</p>
                ...
                <div className="flex justify-between items-center">
                    {certifications && (
                        <div className="flex flex-wrap mr-2">
                            {certifications.map((certificate, index) => (
                                <div key={index} className="mt-2 mx-2 cursor-pointer" onClick={() =>(window.location.href = certificate)}>
                    ...
                    {showEdit && <div>
                            <button className='border bg-green-400 mx-2 p-2 rounded-xl' onClick={()=> navigate('/enquiries/'+_id)}>
                               Enquiries
                            </button>
                        </div>}
                ...
                </div>
                {
                        !showEdit && 
                            <div className='flex flex-col ml-2 mt-8 w-1/2 '>
                                <button className='w-full rounded-xl bg-blue-300 text-black p-2' onClick={handleChat}>Chat</button>
                            </div>
                    }
